...
# ---------------------------------------------------------------------------
# Shared reflection helper for confirm command
# ---------------------------------------------------------------------------

def _reflect_item(item, *, twin, trace_store, calibration_store, experience_store, llm):
    """Record outcome + generate reflection + update experience for one queued item.

    Shared by both --accept-all and interactive accept paths.
...
def cmd_confirm(args):
    ...
    # Both accept-all and interactive need stores + twin
    from twin_runtime.infrastructure.backends.json_file.trace_store import JsonFileTraceStore
    from twin_runtime.infrastructure.backends.json_file.calibration_store import CalibrationStore
    from twin_runtime.infrastructure.backends.json_file.twin_store import TwinStore
    from twin_runtime.infrastructure.backends.json_file.experience_store import ExperienceLibraryStore
    ...
    reflect_kwargs = dict(
        ...
        experience_store=exp_store, llm=llm,
    )
    ...
    if accept_all:
        ...
        for item in pending:
            try:
                _reflect_item(item, **reflect_kwargs)
                accepted += 1
